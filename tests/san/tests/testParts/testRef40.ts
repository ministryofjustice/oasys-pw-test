...
export function demergeAndCheckOffenders(mergeTestData: MergeTestData) {
    ...
    test('Merge tests part 5 - demerge offenders', async ({ oasys, user, offender, san, sns }) => {
        ...
        await sns.testSnsMessageData(mergeTestData.offender1.probationCrn, 'assessment', ['AssSumm', 'OGRS', 'RSR'], 10)
        await sns.testSnsMessageData(mergeTestData.offender2.probationCrn, 'assessment', ['AssSumm', 'OGRS', 'RSR'], 10)
